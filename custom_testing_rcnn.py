...
mmdet.datasets.coco.CocoDataset.CLASSES=('right lung', 'left lung', 'left costophrenic angle', 'trachea',
           'right costophrenic angle', 'cavoatrial junction', 'left mid lung zone', 'left clavicle',
           'right hemidiaphragm', 'aortic arch', 'left hemidiaphragm',
           'right apical zone', 'carina', 'left hilar structures', 'right hilar structures',
           'right clavicle', 'svc', 'upper mediastinum', 'right atrium',
           'right mid lung zone', 'left apical zone', 'left upper lung zone', 'right upper lung zone',
           'left lower lung zone', 'right lower lung zone', 'cardiac silhouette')
...
for i in range(len(test_dataset)-70):
    ...
    #     rect = patches.Rectangle((bbox[0], bbox[1]), bbox[2], bbox[3], linewidth=2, edgecolor=class_colors[class_name],
    #                              facecolor='none')
    #     ax.add_patch(rect)
    for bbox, class_name in zip(pred_bboxes, gt_class_names):
        ...
        ax.add_patch(rect)
        ax.text(bbox[0], bbox[1] - 10, class_name, fontsize=10, color='m')
    ...
    plt.show()
    fig.savefig('./analysis/faster_rcnn_all/output'+str(i)+'.jpg')
