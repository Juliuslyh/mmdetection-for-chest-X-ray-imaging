...
device_yolo = 'cuda:4'  # or 'cpu'
device_ssd = 'cuda:5'  # or 'cpu'
...
class_idxs = [5]  # change this to the desired class index

# loop over the test dataset and visualize the bounding boxes for each image
for i in range(len(test_dataset)-50):
    # get the i-th sample
    ...
    pred_bboxes_yolo = []
    for class_idx in class_idxs:
        pred_bbox = result_yolo[class_idx]
        # Sort the bounding box predictions based on the score in descending order
        sorted_bboxes = pred_bbox[pred_bbox[:, 4].argsort()[::-1]]
        # Take the bounding box with the highest score
        highest_score_bbox = sorted_bboxes[0]

        pred_bboxes_yolo.append([highest_score_bbox])
    pred_bboxes_yolo = np.concatenate(pred_bboxes_yolo)

    pred_bboxes_ssd = []
    for class_idx in class_idxs:
        pred_bbox = result_ssd[class_idx]
        # Sort the bounding box predictions based on the score in descending order
        sorted_bboxes = pred_bbox[pred_bbox[:, 4].argsort()[::-1]]
        # Take the bounding box with the highest score
        highest_score_bbox = sorted_bboxes[0]
    ...
    plt.show()
    fig.savefig('./analysis/cavoatrial junction_all/output'+str(i)+'.jpg')
